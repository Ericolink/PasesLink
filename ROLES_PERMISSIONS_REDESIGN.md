...
**Deliberadamente NO implementado en esta fase**: cambiar el rol de un colaborador ya aceptado (hay que revocar y re-invitar); badge de rol en `EventTicketCard`/`Dashboard`. La consolidación de `CoOrganizerPanel` en `CollaboratorPanel` sí se hizo, pero después, a pedido explícito del usuario — ver "Fusión CoOrganizer+Administrador" más abajo. `ConcessionStaffPanel` (encargados legacy de ventas) sigue aparte a propósito, ver esa misma sección.

### 4.1 Fusión CoOrganizer + colaborador Administrador (2026-08-13, a pedido explícito del usuario)

Un coorganizador legacy y un colaborador de rol `administrador` ya eran funcionalmente equivalentes (`isCoOrg`/`hasAccess` true para ambos, mismo nivel de acceso al dashboard completo) — solo se mostraban como dos conceptos separados en la UI, con dos botones y dos paneles distintos en `EventDetail.tsx`. Se fusionaron en uno solo:

- **`CollaboratorPanel.tsx` reescrito**: ahora recibe tanto `coOrg` (`useCoOrganizers`) como `collab` (`useCollaborators`) y construye una lista combinada de ambos sistemas, **agrupada por rol** (Administrador, Recepción, Caja, Ventas, Preparación, Comunidad — pedido explícito: "quiero ver en cada apartado a qué usuario tengo asignado"). Un coorganizador legacy aparece bajo el grupo "Administrador" junto con cualquier colaborador nuevo-estilo de ese mismo rol. Las entradas legacy conservan el editor de permisos fino (`CoOrganizerPermissionsEditor`, expandible al tocar el email) porque sí tienen valor real (permisos por-uid, a diferencia del preset fijo del sistema nuevo); las entradas nuevas no lo tienen (cambiar su rol sigue siendo revocar+reinvitar).
- **Alta unificada**: el selector de rol (ya existía, 6 opciones) es ahora la ÚNICA forma de invitar — incluido el rol Administrador. Se retiró de la UI la generación de enlaces del sistema legacy (`useCoOrganizers.ts` perdió `handleGenerateInvite`/`invite`/`inviteLoading`/`inviteError`; `src/firebase/coOrganizerInvites.ts` perdió `createCoOrganizerInvite`/`buildCoOrganizerInviteUrl`) — pero **`acceptCoOrganizerInvite` se conservó intacto**, tanto la Cloud Function como el wrapper cliente y la ruta `/co/:eventId/:token`: un enlace ya compartido antes de esta fusión tiene que seguir siendo canjeable.
- **`CoOrganizerPanel.tsx` eliminado** (absorbido por completo, sin llamador restante).
- **Un solo botón/ícono** en `EventDetail.tsx` ("Colaboradores", `IconShield`) con el contador combinado (legacy + nuevo) en vez de dos botones con contadores separados.
- **Bug real corregido de paso**: la sección "Salir del evento" siempre llamaba a `leaveCoOrganizer` (legacy) sin importar el sistema real del uid — un colaborador nuevo-estilo de rol Administrador que intentara salir habría escrito sobre el mapa equivocado (`coOrganizersMap`, donde no tiene entrada) en vez de `collaborators`. `handleLeave` ahora detecta el sistema real (`event.collaborators?.[uid]` vs `coOrganizersMap`) y llama a la función correcta.

**Deliberadamente fuera de esta fusión**: `ConcessionStaffPanel.tsx` (encargados legacy de "Ventas del evento", vía `concessions.concessionsStaffMap`) sigue siendo un panel aparte, sin fusionar — el pedido del usuario nombró específicamente "coorganizador y colaborador administrador", no ventas. Sus miembros tampoco aparecen en la vista agrupada por rol del `CollaboratorPanel` unificado.

Verificado: `tsc -b`, `eslint`, `npm run test` (183/183), `npm run build`, `knip` — todo verde. Sin tests de componente nuevos (mismo criterio ya establecido: ninguno de estos componentes tuvo cobertura de ese tipo antes). **Sin probar en navegador** — este entorno no tiene sesión de Firebase válida para ejercer el flujo autenticado end-to-end (ver `feedback_no_prod_firebase_smoke_tests`); queda pendiente de que el usuario lo pruebe manualmente en staging.
...
## 6.1 Bug crítico encontrado en producción (2026-08-13) — `mapEvent()` nunca copiaba `collaborators`

El usuario probó el flujo real compartiendo un enlace de invitación de rol Preparación: el destinatario vio el evento listado en su `/dashboard` (confirma que el fix de `subscribeToUserEvents` de la Fase 4 funciona — esa query filtra sobre el campo crudo de Firestore), pero al entrar al evento no tenía ningún acceso.

**Causa raíz**: `mapEvent()` (`src/firebase/events.ts`) construye el objeto `EventData` que consume toda la app copiando cada campo EXPLÍCITAMENTE desde el documento crudo de Firestore (no hace `{ ...data }`) — y `collaborators` nunca se agregó a esa lista al crear el campo en la Fase 1. Resultado: `event.collaborators` daba `undefined` en TODA la app cliente (dashboard, `EventDetail`, redirect por rol, `useEventPermissions`) sin importar lo que hubiera realmente en Firestore — mientras que Firestore Rules, `hasPermission()` (Cloud Functions) y las queries crudas de Firestore (como la del dashboard) funcionaban perfectamente, porque ninguno de esos tres pasa por `mapEvent()`. Ninguno de los tests automatizados de las Fases 1-6 lo detectó porque todos construyen su propio `EventData`/`DocumentData` de prueba a mano, sin pasar nunca por el mapper real.

**Ya había pasado exactamente esto antes** (2026-07-31, con el campo `concessions` al agregar el módulo de ventas) — el propio archivo `src/firebase/events.test.ts` ya tenía un test de regresión documentando ese primer incidente. Cualquier campo nuevo que se agregue a `EventData` en el futuro necesita sumarse explícitamente a `mapEvent()` — no hay ningún mecanismo que lo fuerce a nivel de tipos (TypeScript no obliga a que `mapEvent` copie todos los campos declarados en `EventData`, solo a que lo que sí copia tenga el tipo correcto).

**Fix**: `mapEvent()` ahora copia `collaborators` (nueva función `mapCollaborators()`), normalizando `invitedAt` a `number` con un helper nuevo (`toMillisOrNumber`) porque ese campo puede llegar como Firestore `Timestamp` real (`acceptCollaboratorInvite.ts`, vía `FieldValue.serverTimestamp()`) o ya como `number` (`scripts/backfill-collaborators-from-legacy.mjs`, que usa `Date.now()`). `EventSchema` (`src/types/schemas.ts`) también ganó la entrada correspondiente (`CollaboratorEntrySchema`) para que el chequeo de forma en dev (`warnIfInvalidShape`) también lo cubra. 4 tests nuevos de regresión en `events.test.ts`, mismo patrón que el caso de `concessions`.

## 6.2 Ajustes de UX de roles pedidos tras la prueba real (2026-08-13)

- **Recepción ahora tiene `hasAccess: true`** (antes solo Administrador) — ve el dashboard de `EventDetail` completo pero de solo lectura: cada acción de edición sigue gateada por su propio permiso (`editEvent`/`manageCoOrganizers`/`addGuests`, todos `false` en su preset), así que ni edita el evento ni gestiona colaboradores/invitados — pero sí ve los datos del evento, la lista de invitados, y puede escanear/marcar pagos. Antes, al no tener `hasAccess`, cualquier colaborador de rol Recepción se redirigía directo a `/scan` sin poder ver nada más — pedido explícito del usuario para cambiar eso.
- **`confirmPayments` pasó a ser parte del preset BASE de Recepción** (antes era `permissionOverrides` opcional, decisión de la propuesta original) — pedido explícito: "marcar invitados como pagados o no pagados" debe ser una capacidad de base, no algo que el anfitrión tenga que activar a mano evento por evento. Sigue siendo angostable con `permissionOverrides: { confirmPayments: false }` si un evento puntual quiere que solo Caja confirme pagos.
- **Efecto secundario conocido, no corregido a propósito**: como `confirmPayments` también gatea (por diseño heredado del sistema de coorganizador legacy) la visibilidad del acceso directo a "Ventas" y la posibilidad de confirmar pedidos de concesiones, Recepción con este cambio también puede ver el atajo "Ventas" y confirmar pedidos de ventas si el evento tiene el módulo activo — no es lo que se pidió explícitamente, pero separar "confirmar pago de entrada" de "confirmar pago de venta" en permisos distintos es un cambio más grande (afecta rules/functions/tipos en los 3 lugares) que no se hizo en esta pasada. Si molesta en la práctica, es la primera cosa a resolver.
- **Preparación → `/kitchen` y Ventas → `/menu`** ya estaban implementados desde la Fase 4 (el redirect por rol angosto) — no hacía falta código nuevo para esos dos, solo el fix del bug de `mapEvent()` de arriba para que `perms.prepareOrders`/`perms.manageConcessions` dejaran de ser siempre `false`.
