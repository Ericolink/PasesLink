...
**Rol Comunidad**: evaluado y **no incluido en la v1** de esta propuesta por decisión explícita (ver §5) — `wall.moderate` permanece como permiso dentro de Administrador. Queda documentado como extensión trivial a futuro: el permiso ya existe (`moderateWall`), solo falta aislarlo en un preset propio si en algún momento se necesita un colaborador que *solo* modere el muro.
...
**Fase 5 (opcional, evaluar según uso real) — Rol Comunidad.** Aislar `wall.moderate` en un preset propio si aparece la necesidad concreta de un colaborador que solo modere el muro sin ser Administrador.

**Fase 6 — Limpieza.** Retirar `coOrganizersMap`/`coOrganizerPermissions`/`concessionsStaffMap` como campos de escritura una vez confirmado en producción que todo colaborador activo tiene entrada equivalente en `collaborators` (mismo período de espera y verificación manual que ya se usó para retirar el fallback `exists()` de `isAdmin()`).
