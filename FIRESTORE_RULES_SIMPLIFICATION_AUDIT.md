...
4. ✅ **Implementado (2026-08-01), pendiente deployar + narrow de rules.** `createConcessionOrder` → Callable Function (`functions/src/callable/createConcessionOrder.ts` + servicio puro `functions/src/concessions/createConcessionOrder.ts`) que recalcula precio/stock desde `concessionsCatalog` real dentro de la transacción — nadie que hable directo contra Firestore puede fabricar un total que no coincida con el catálogo. Junto con esto, también se migró `cancelConcessionOrder` (`functions/src/callable/cancelConcessionOrder.ts` + `functions/src/concessions/cancelConcessionOrder.ts`, cancelación del organizador que libera stock reservado) porque comparte la tercera función de contabilidad de stock (`isValidConcessionStockRelease`) que había que cerrar junto con las otras dos. Verificación de dueño del pedido (`guestLockTokensOk`, puerto de la homónima en rules) y de permiso de organizador (`canManageConcessions`, nuevo en `functions/src/lib/permissions.ts`) ambas del lado servidor. `src/firebase/concessions.ts` — `createConcessionOrder`/`cancelConcessionOrder` son ahora wrappers delgados sobre `httpsCallable`, misma firma exportada (no tocó `GuestConcessionsModal.tsx`/`ConcessionOrdersPanel.tsx`). `confirmConcessionOrderPayment`/`rejectConcessionOrderPayment`/`cancelOwnConcessionOrder`/`advanceConcessionFulfillment`/`revertConcessionFulfillment` quedan sin migrar (fuera de alcance, sin aritmética de negocio). 166 tests en `functions/` (antes 156) + 229 en `test:firebase` (antes 236, se sacaron los 7 que ahora viven en `functions/src/concessions/*.test.ts`) + build/lint/tsc OK. **Falta:** desplegar las 2 Cloud Functions, confirmar en producción, y solo entonces angostar `firestore.rules` (`isValidConcessionOrderCreate`/`isValidConcessionStockDecrement`/`isValidConcessionSoldCountBump`/`isValidConcessionStockRelease`, rules:746-817, más las ramas de `concessionsCatalog`/`concessionsOrders` que las usan).
