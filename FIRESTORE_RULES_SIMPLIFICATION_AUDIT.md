...
4. ✅ **Implementado (2026-08-01), rules angostadas (2026-08-02) — pendiente deployar.** `createConcessionOrder` → Callable Function (`functions/src/callable/createConcessionOrder.ts` + servicio puro `functions/src/concessions/createConcessionOrder.ts`) que recalcula precio/stock desde `concessionsCatalog` real dentro de la transacción — nadie que hable directo contra Firestore puede fabricar un total que no coincida con el catálogo. Junto con esto, también se migró `cancelConcessionOrder` (`functions/src/callable/cancelConcessionOrder.ts` + `functions/src/concessions/cancelConcessionOrder.ts`, cancelación del organizador que libera stock reservado) porque comparte la tercera función de contabilidad de stock (`isValidConcessionStockRelease`) que había que cerrar junto con las otras dos. Verificación de dueño del pedido (`guestLockTokensOk`, puerto de la homónima en rules) y de permiso de organizador (`canManageConcessions`, nuevo en `functions/src/lib/permissions.ts`) ambas del lado servidor. `src/firebase/concessions.ts` — `createConcessionOrder`/`cancelConcessionOrder` son ahora wrappers delgados sobre `httpsCallable`, misma firma exportada (no tocó `GuestConcessionsModal.tsx`/`ConcessionOrdersPanel.tsx`). `confirmConcessionOrderPayment`/`rejectConcessionOrderPayment`/`cancelOwnConcessionOrder`/`advanceConcessionFulfillment`/`revertConcessionFulfillment` quedan sin migrar (fuera de alcance, sin aritmética de negocio, ver §2.2).
   **Cierre (2026-08-02):** dos huecos reales encontrados y corregidos antes de angostar rules — (a) `createConcessionOrder` no validaba `quantity` (una línea negativa inflaba `stockRemaining` en vez de descontarlo, y podía dejar un `totalMinorUnits` negativo) ni deduplicaba `itemId` repetido dentro del mismo pedido (dos líneas del mismo producto pisaban el `stockRemaining` que dejaba la otra dentro de la transacción, porque a diferencia de `soldCount` — un `FieldValue.increment` — la escritura de `stockRemaining` no es acumulativa entre dos `tx.update` al mismo doc); ahora `normalizeLines()` valida `quantity` entero 1-50 por línea (mismo techo que tenía `isValidConcessionStockDecrement`), máximo 30 líneas, y fusiona duplicados sumando cantidades. (b) `currency` se guardaba tal cual la mandaba el cliente; ahora se resuelve siempre desde `event.concessions.currency` (con el mismo `input.currency` solo como respaldo defensivo para datos preexistentes al campo). **Angostado `firestore.rules`:** se retiraron `isValidConcessionOrderCreate`/`isValidConcessionStockDecrement`/`isValidConcessionSoldCountBump`/`isValidConcessionStockRelease` (rules:751-830 antes del cierre) y las 3 ramas que las usaban; `concessionsOrders.create` y `concessionsFulfillment.create` pasaron a `allow create: if false` explícito (ambos documentos ya se crean SIEMPRE dentro de la misma transacción de la Callable, vía Admin SDK, que bypassea rules). `isGuestOrderActor`/`isValidConcessionItem` se conservan (siguen usados por el autoservicio del invitado — comprobante/autocancelación — y por el alta/edición del catálogo del organizador). 11 tests nuevos en `functions/src/concessions/createConcessionOrder.test.ts` (línea negativa, no-entera, por-encima-del-techo, líneas duplicadas, ítem inexistente, ítem archivado, precio inyectado por el cliente ignorado, moneda inyectada por el cliente ignorada) + 4 tests nuevos en `src/firebase/__tests__/concessions.rules.test.ts` (`assertFails` sobre escritura directa sintácticamente válida sobre `concessionsOrders`/`concessionsCatalog`/`concessionsFulfillment`, demostrando que ya no queda ninguna rama de rules que la autorice). **Falta:** desplegar las 2 Cloud Functions Y las rules nuevas — en ese orden (ver §7) — y confirmar el flujo de checkout en producción.

**Fase C — Optimización estructural (puede correr en paralelo a A/B, es independiente).**
5. ✅ **Implementado (2026-08-01) en estado transicional — pendiente deploy + backfill + confirmar antes de retirar el fallback.** `isAdmin()` (rules) ahora acepta `request.auth.token.get('admin', false) == true` ADEMÁS del `exists(/admins/$(uid))` de siempre (cambio puramente aditivo, a diferencia de las Fases A/B: nada se quitó todavía, así que no hace falta esperar confirmación en producción para que este commit sea seguro). `functions/src/triggers/onAdminWritten.ts` (nuevo, sin test directo — mismo criterio que `onCapacityFreed`, el otro trigger del proyecto) sincroniza el claim cuando se crea/borra un documento en `admins/{uid}`. `scripts/backfill-admin-claims.mjs` (nuevo, sin test — mismo patrón que los demás `backfill-*.mjs`) otorga el claim a los admins que ya existían antes del trigger. Test nuevo en `admin.test.ts` que verifica la rama del claim usando `authenticatedContext(uid, { admin: true })`, sin necesitar el emulador de Auth. 230 tests en `test:firebase` (antes 229), 166 en `functions/` sin cambios, build/lint/tsc OK. **Falta:** desplegar el trigger, correr el backfill una vez en producción, esperar 1-2 semanas confirmando (Firebase Console → Authentication → Custom claims) que todos los admins actuales tienen el claim, y **recién entonces** retirar la rama `exists(...)` de `isAdmin()` en un commit aparte.

**Actualización (2026-08-01, mismo día):** se cerró el hueco del lado cliente que dejaba esta fase — nada en la app forzaba nunca un refresh del ID token, así que un admin recién agregado solo veía el claim reflejado en su token tras el refresh automático del SDK (hasta 1h) o cerrar sesión. `useIsAdmin` (antes una lectura única con `getDoc`) pasó a un `onSnapshot` sobre `admins/{uid}`: en cada cambio compara `snap.exists()` contra el claim ya cacheado en el token (`getIdTokenResult()`, sin forzar) y, si difieren, llama `getIdToken(true)` para sincronizarlo — cubre alta, baja y reactivación sin pedirle nada al usuario. `isAdmin` (el valor que devuelve el hook) sigue reflejando el doc, no el claim, para no depender de que ese refresh ya haya terminado al pintar la UI la primera vez. Efecto secundario: `checkIsAdmin` (`src/firebase/admin.ts`) quedó sin ningún llamador — se borró junto con los imports de `getDoc`/`doc` que dejó sueltos. 230 tests en `test:firebase` (sin cambios, esta pieza es de cliente puro), `tsc`/`eslint`/`npm test` OK.

**Fase D — Contadores agregados (deuda técnica relacionada, no bloqueante).**
6. ✅ **Implementado (2026-08-01), ampliado (2026-08-05), pendiente deployar.** Reconciliación en dos capas: barrido liviano (`functions/src/scheduled/reconcileDirtyGuestCounters.ts`, cada 10 min) que solo toca eventos marcados `countersDirty` por el trigger `functions/src/triggers/onGuestWritten.ts` (cualquier escritura en `guests/{guestId}`), y barrido completo (`functions/src/scheduled/reconcileGuestCounters.ts`, ahora semanal en vez de diario — el liviano cubre el caso común) como red de seguridad final. Lógica pura compartida en `functions/src/reconciliation/reconcileGuestCounters.ts`, recalcula `guestCount`/`peopleCount`/`paidCount`/`rsvpYesCount`/`rsvpNoCount`/`rsvpPendingCount`/`checkedInCount`/`occupancyCount` desde `guests/{guestId}` y corrige cualquier drift — mismo criterio que usaban `scripts/backfill-paid-count.mjs`/`backfill-rsvp-counts.mjs` (retirados, ya redundantes), ahora automático y recurrente en vez de manual. **`checkedInCount`/`occupancyCount` YA NO están fuera de alcance** (a diferencia de la primera versión de este ítem): se agregó `walkInNetCount` (`src/firebase/counters/config.ts`), un ledger separado que `walkIn`/`walkOut` mantienen aparte — la porción de esos dos contadores que corresponde a invitados identificados se recalcula desde `guests/`, y la porción de walk-ins (que no crean documento de invitado) se toma de ese ledger, la única fuente de verdad posible para esa parte. La escritura final es segura ante concurrencia (transacción que revalida `countersDirtyAt` contra el valor visto al escanear, aborta sin escribir si cambió en el medio) e idempotente. Backfill único `scripts/backfill-walkin-net-count.mjs` para poblar `walkInNetCount` en eventos existentes a partir del `occupancyCount` ya guardado. Sin test para los wrappers `onSchedule`/`onDocumentWritten` (mismo criterio ya establecido para `onAdminWritten`/`onCapacityFreed`), sí hay tests directos de la lógica pura y del guard de concurrencia. `firestore.rules` sí se tocó esta vez: `walkInNetCount` se agregó al `hasOnly` de la rama `scanQr` (walkIn/walkOut). 240 tests en `test:firebase`/`functions` (antes 172 combinados), build/lint/tsc OK. **Falta:** desplegar Cloud Functions + rules, correr el backfill una vez.
