...
6. ✅ **Implementado (2026-08-01), pendiente deployar.** Reconciliador diario (`functions/src/scheduled/reconcileGuestCounters.ts`, Cloud Scheduler 04:00 UTC + lógica pura testeable en `functions/src/reconciliation/reconcileGuestCounters.ts`) que recalcula `guestCount`/`peopleCount`/`paidCount`/`rsvpYesCount`/`rsvpNoCount`/`rsvpPendingCount` desde `guests/{guestId}` y corrige cualquier drift — mismo criterio que ya usaban `scripts/backfill-paid-count.mjs`/`backfill-rsvp-counts.mjs`, ahora automático y recurrente en vez de manual. **`checkedInCount`/`occupancyCount` quedaron explícitamente FUERA de alcance** (a diferencia de lo que sugería este mismo ítem del roadmap): `walkIn`/`walkOut` los incrementan sin crear ningún documento de invitado, así que recalcularlos desde `guests/` borraría esa porción — sería una regresión, no una corrección; siguen dependiendo únicamente de `increment()` en caliente. Sin test para el wrapper `onSchedule` (mismo criterio ya establecido para `onAdminWritten`/`onCapacityFreed` — ningún trigger/scheduled de este proyecto tiene test propio). No toca `firestore.rules` ni código de cliente — 172 tests en `functions/` (antes 166), sin cambios en `test:firebase`/`test`/build de raíz. **Falta:** desplegar la Cloud Function.
