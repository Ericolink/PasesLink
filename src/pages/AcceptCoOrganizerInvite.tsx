...
import { acceptCoOrganizerInvite } from '../firebase/coOrganizerInvites'
import { useAuth } from '../hooks/useAuth'
import { useDocumentTitle } from '../hooks/useDocumentTitle'
import { ScreenHeader } from '../components/ScreenHeader'
import { CrownLoader } from '../components/CrownLoader'
import { AccessibleButton } from '../components/accessibility/AccessibleButton'
import { IconBan, IconCheckCircle, IconUsers } from '../components/accessibility/AccessibleIcon'
...
const OUTCOME_MESSAGE: Record<Exclude<Outcome, 'success' | 'already_member'>, string> = {
  ...
  full: 'Este evento ya alcanzó el máximo de coorganizadores permitidos.',
  error: 'No pudimos procesar la invitación. Intenta de nuevo en unos minutos.',
}

// Pantalla de aceptación del enlace de invitación de coorganizador (rediseño
// del Dashboard del Evento — reemplaza el alta directa por email como flujo
// principal). Detrás de ProtectedRoute (ver App.tsx): sin sesión, redirige a
// /login conservando este mismo destino (state.from, mismo mecanismo que ya
// usa cualquier otra ruta protegida) — así que acá siempre hay un
// `user` disponible antes de intentar canjear el token.
export function AcceptCoOrganizerInvite() {
  ...
  useDocumentTitle(event ? `Coorganizador · ${event.name}` : 'Invitación de coorganizador')
  ...
  async function handleAccept() {
    ...
    setAccepting(true)
    try {
      const result = await acceptCoOrganizerInvite(eventId, token)
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8">
      <ScreenHeader title="Invitación de coorganizador" backTo="/dashboard" />
        ...
        <div className="p-5 text-center">
          {!outcome && (
            <>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Te invitaron a coorganizar</p>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300 mb-5">
                Como coorganizador vas a poder gestionar invitados, escanear pases y más, según los permisos que te
                otorgue quien organiza este evento.
              </p>
              <AccessibleButton onClick={handleAccept} disabled={accepting} className="w-full">
                {accepting ? 'Uniéndote…' : 'Unirme como coorganizador'}
              </AccessibleButton>
              {!user && (
          ...
          {(outcome === 'success' || outcome === 'already_member') && (
            <>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                {outcome === 'already_member' ? 'Ya eras coorganizador de este evento.' : 'Ahora eres coorganizador de este evento.'}
              </p>
              <Link
                to={`/events/${eventId}`}
