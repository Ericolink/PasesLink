...
const TYPE_CONFIG: Record<WallMessageType, { label: string; emoji: string; color: string }> = {
  comment: { label: 'Comentario', emoji: '💬', color: 'bg-blue-100 text-blue-700' },
  question: { label: 'Pregunta', emoji: '❓', color: 'bg-yellow-100 text-yellow-700' },
  music: { label: 'Música', emoji: '🎵', color: 'bg-purple-100 text-purple-700' },
    ...
    <div className="max-w-xl mx-auto px-4 py-6 min-h-screen">
      ...
      <form onSubmit={handlePost} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 mb-5 space-y-3">
          ...
          {(Object.keys(TYPE_CONFIG) as WallMessageType[]).map((t) => (
            <button
              ...
              className={`text-xs rounded-full px-3 py-1 font-medium transition-all ${
                type === t ? TYPE_CONFIG[t].color + ' ring-2 ring-offset-1 ring-current' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
              }`}
            >
        ...
        <textarea
          ...
          placeholder={`${TYPE_CONFIG[type].emoji} Escribe tu ${TYPE_CONFIG[type].label.toLowerCase()}...`}
          rows={2}
          ...
          <div key={msg.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 animate-fade-in-up">
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                <span className={`text-xs rounded-full px-2 py-0.5 font-medium ${TYPE_CONFIG[msg.type].color}`}>
