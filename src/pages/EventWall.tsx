...
export function EventWall() {
  ...
  if (!nameConfirmed && !isOwner && !user) {
    ...
    return event?.templateId === 'cowboy' || event?.templateId === 'graduation' ? (
      <InvitationThemeRoot templateId={event.templateId} accentOverride={event.accentColor} themeOverrides={event.themeOverrides} communityTemplateVars={event.communityTemplateSnapshot?.vars} className="min-h-dvh flex items-center justify-center p-4">
        {nameGateContent}
      </InvitationThemeRoot>
    ) : (
  ...
  return event?.templateId === 'cowboy' || event?.templateId === 'graduation' ? (
    <InvitationThemeRoot templateId={event.templateId} accentOverride={event.accentColor} themeOverrides={event.themeOverrides} communityTemplateVars={event.communityTemplateSnapshot?.vars} className="max-w-xl mx-auto px-4 py-6 min-h-dvh">
      {content}
    </InvitationThemeRoot>
  ) : (
